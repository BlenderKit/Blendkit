...
from . import paths, persistent_preferences
...
class TestUpdateScrollButtonIcons(unittest.TestCase):
    ...
    def _make_dummy_operator(**overrides):
        return SimpleNamespace(
            ...
            button_scroll_down=Mock(),
            button_scroll_up=Mock(),
        )

    def test_single_row_uses_horizontal_arrows(self):
        dummy = self._make_dummy_operator(hcount=1)
    ...
    def test_multi_row_uses_vertical_arrows(self):
        dummy = self._make_dummy_operator(hcount=3, bar_height=288)
        asset_bar_op.BlenderKitAssetBarOperator.update_scroll_button_icons(dummy)
    ...
    def test_preserves_aspect_ratio(self):
        dummy = self._make_dummy_operator(hcount=2, scroll_width=30, bar_height=180)
        asset_bar_op.BlenderKitAssetBarOperator.update_scroll_button_icons(dummy)
